...
The snapshot separates producer policy (`voting`, `advisory`, `unknown`) from observed coverage (`direct`, `required-aggregate`, `uncovered`, `advisory`, `unknown`). A required aggregate needs explicit `all-needs` failure-propagation evidence. Dependency ancestry alone does not prove protection. Findings remain the audit result; suggestions are review-only and require a new audit after any human change. No YAML patch or GitHub write is performed.
